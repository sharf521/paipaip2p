...
function magic_function_linkages($parse_var){
	...
	$display.="\$nid='$nid';";
		$display .="if(\$nid=='select_biao_type'){";
			$display .="\$site_biaotype=explode(':',\$this->magic_vars['_G']['site_biaotype']);";
			
			$display .="foreach(\$result as \$i=>\$v){
				if(!in_array(\$v['value'],\$site_biaotype) && \$v['value']!='all'){unset(\$result[\$i]);}	}";
		$display.="}";
