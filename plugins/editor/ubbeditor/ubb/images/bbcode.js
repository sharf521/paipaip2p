...
function bbcode2html(str) {
	...
	if(_allowFaces) {
		for(id in faces) {
			re = new RegExp(addslashes(faces[id]['code']), "g");
			str = str.replace(re, '<img src="./images/faces/' + faces[id]['url'] + '" border="0" smilieid="' + id + '" alt="' + faces[id]['code'] + '" />');
		}
	}
	...
	if(_allowImgCode) {
		...
		//str = str.replace(/\[imgfile\](\d+)\[\/imgfile\]/ig, function ($1, $2) {return '';});
...
function html2bbcode(str) {
	...
	if( trim(str) == '') {
		str = str.replace(/<img[^>]+smilieid=(["']?)(\d+)(\1)[^>]*>/ig, function($1, $2, $3) {return faces[$3]['code'];});
...
function imgtag(attributes) {
	...
	//re = /aid=(["']?)imgfile_(\d+)(\1)/i;
	//var matches = re.exec(attributes);
	//if(matches != null) {
	//	return '[imgfile]' + matches[2] + '[/imgfile]';
	//}
...
function listtag(listoptions, text, tagname) {
	text = text.replace(/<li>(([\s\S](?!<\/li))*?)(?=<\/?ol|<\/?ul|<li|\[list|\[\/list)/ig, '<li>$1</li>') + (isOpera ? '</li>' : '');
