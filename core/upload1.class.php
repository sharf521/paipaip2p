...
class upload
{
    ...
    var $all_type = array(
 	 ...
 	 var $data_dir="data/upfiles";// 上传的文件目录
    /**
     * 构造函数
     */
    function upload()
    ...
    {
		 if((empty($this->mask_word) && empty($this->mask_img)) || $this->uploadImgmask==0)
        {
	...
	function setUploadImgtype($type){
		if ($type=="") $type = "gif|png|jpg";
		$this->uploadImgtype = explode("|",$type);
	}
	
	function setUploadImgmaxsize($size=500){
		$this->uploadImgmaxsize = $size;
	}
	
	function setUploadImgdir($dir="images"){
		$this->uploadImgdir = $this->data_dir."/".$dir."/";
	}
	
	function setUploadMask($mask){
		$this->uploadImgmask = $mask;
	}
	function setUploadMasktype($type=0){
		$this->uploadImgmasktype = $type;
	}
	function uploadImg($file){
		if (is_array($_FILES[$file]['name'])){
			...
			foreach($_FILES[$file]['name'] as $i => $value){
				if ($value!=""){
					$error = "";
					if ($_FILES[$file]['size'][$i]==0)	$error = -1;
					if(!in_array(strtolower(substr($_FILES[$file]['name'][$i],-3,3)),$this->uploadImgtype)) $error = -1;
					if(strpos($_FILES[$file]['type'][$i],'image')===false) $error = -1;
					if($_FILES[$file]['size'][$i] > $this->uploadImgmaxsize*1024) $error = -2;
					if($_FILES[$file]['error'][$i] !=0 ) $error = -3;
				
					$this->_mkdirs($this->uploadImgdir);
					
					$newFile=md5(time()) . rand(0, 100) . rand(0, 100) .$i.substr($_FILES[$file]['name'][$i],-4,4);
					$oldFile = $_FILES[$file]['name'][$i];
					$allFile=$this->uploadImgdir.$newFile;
					...
					if ($this->uploadImgmask==1){
						if ($this->uploadImgmasktype==0){
							$this->mask_word = "";
						}  
					}
					$this->setSrcImg($this->uploadImgdir.$newFile);
					$this->setDstImg($this->uploadImgdir.$newFile); 
					$this->createImg($this->pic_w,$this->pic_h);  
					if ($error==""){
						$_result[] = $this->uploadImgdir.$newFile;
					}
				}
			}
			return $_result;
		}else{
			if ($_FILES[$file]['size']==0)	return -1;
			
			if(!in_array(strtolower(substr($_FILES[$file]['name'],-3,3)),$this->uploadImgtype)) return -1;
			if(strpos($_FILES[$file]['type'],'image')===false) return -1;
			if($_FILES[$file]['size']> $this->uploadImgmaxsize*1024) return -2;
			if($_FILES[$file]['error'] !=0 ) return -3;
			
			$this->_mkdirs($this->uploadImgdir);
			
			$newFile=md5(time()) . rand(0, 100) . rand(0, 100) . substr($_FILES[$file]['name'],-4,4);
			...
			$this->setDstImg($this->uploadImgdir.$newFile); 
			$this->createImg($this->pic_w,$this->pic_h); 
			return $this->uploadImgdir.$newFile;
		}
	}
