...
class guofubaoPayment {
    ...
    public static function ToSubmit($payment){
		...
		$backgroundMerUrl = $payment['notify_url']; 
                //$backgroundMerUrl = $payment['return_url'];      
		...
		$tranDateTime = $Date;     
		$virCardNoIn = '0000000002000119698';      
		$tranIP = $ipAddr;           
		...
		$signValue='version=['.$version.']tranCode=['.$tranCode.']merchantID=['.$merchantID.']merOrderNum=['.$merOrderNum.']tranAmt=['.$tranAmt.']feeAmt=['.$feeAmt.']tranDateTime=['.$tranDateTime.']frontMerUrl=['.$frontMerUrl.']backgroundMerUrl=['.$backgroundMerUrl.']orderId=[]gopayOutOrderId=[]tranIP=['.$tranIP.']respCode=[]VerficationCode=[jk123jkl980kasdfmjdf]';
