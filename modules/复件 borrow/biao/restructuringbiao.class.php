...
class restructuringbiaoClass extends biaotypeClass{
	...
	function vouch($data = array()){
		...
		//因为客户要改成只使用可用余额担保，所以这里判断能否担保的时候，直接只比较可用余额
		//担保是用投资担保额度和可提现金额担保
...
/*		if ($vouch_list==""){
		...
		if($vouch_collection_status==2)////说明保担己经转成债权
		{
			...
			$sql="select sum(has_percent) as has_percent,creditor_id as user_id from {borrow_right} where borrow_id= {$borrow_id} and status=1 group by creditor_id";
