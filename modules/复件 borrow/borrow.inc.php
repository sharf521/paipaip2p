...
// 		$msg = array("验证码错误");
	if(!isset($_POST['id'])){
		...
		//这个更新不需要事务
		$sql = "update  {borrow}  set processing=1,current_userid={$user_id}  where id={$borrow_id} and processing = 0 and current_userid = -1 ";
...
// 		}
		...
		//liukun add for bug 486 begin
		//处理完后，恢复到可以再次处理的状态
		$sql = "update  {borrow}  set processing=0,current_userid=-1  where id={$borrow_id} and processing = 1 and current_userid = $user_id ";
		$mysql->db_query($sql);
		//liukun add for bug 486 begin
	}
	if ($result !== true){
...
elseif ($_U['query_type'] == "buyback"){
	...
	if(1==2){
		...
		elseif ($can_interest_month <= 0){
			...
			//liukun add for bug 52 begin
			fb($borrow_style, FirePHP::TRACE);
			fb($interest, FirePHP::TRACE);
