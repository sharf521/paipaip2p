...
class borrowClass extends amountClass{
	...
	function GetListIndex($data = array()){
		...
		*/
		...
		//判断是不是只显示分站自己的标
		$sql = "select * from  {subsite}  where `id`={$data['areaid']} ";
		$subsite_result = $mysql ->db_fetch_array($sql);
		if ($subsite_result['only_show_sitebiao'] == 1){
			$_sql .= " and p1.areaid = {$data['areaid']} ";
		}
	...
	function Add($data = array()){
		...
		if ($ishappy==1){
			...
			$freeze_happy_interest = ceil($freeze_happy_interest*100)/100;
	...
	public static function Repay($data = array()){
		...
		try{
			...
			if (1!=2){
				...
				{
					if($borrow_repayment_result['status'] ==2)//网站己经垫付
					{						
						if($value['origin_creditor_level'] !=2 && empty($value['repay0orders']))
						{
							continue;	
						}
						else
					...
					//4舍5舍
					$repay_account = round_money($borrow_repayment_result['repayment_account'] * $value['has_percent'] / 100);
					$repay_capital = round_money($borrow_repayment_result['capital'] * $value['has_percent'] / 100);
					$repay_interest = round_money($borrow_repayment_result['interest'] * $value['has_percent'] / 100);
					$repay_late_interest = round_money($late_customer_interest * $value['has_percent'] / 100);
					...
					$account_log['money'] = round_money($borrow_repayment_result['repayment_account'] * $value['has_percent'] / 100);
			...
			//print_r($borrow_repayment_result);
			//exit();	
	...
	function GetAlienateRepaymentList($data = array()){
		...
		foreach ($list as $key => $value){
			...
			$_level=array('普通用户保障','VIP级别保障','不保障');			
				...
				foreach ($borrow_right_result as $key => $value){
					...
					if($advance_account<=0  || $advance_rate==0)
					{
						...
						if($borrow_repayment_result['repayment_account'] > $advance_account || $advance_rate < 1){
							...
							$log['money'] = round_money($borrow_repayment_result['repayment_account'] * $value['has_percent'] / 100) - round_money($advance_account * $value['has_percent'] * $advance_rate / 100);
							$log['total'] = $account_result['total']-$log['money'];
			...
			if ($status == 3){
				...
				foreach ($tender_result as $key => $value){
					...
					foreach ($interest_result as $key => $_interest){
						...
						//$total_collection += round_money($_interest['repayment_account'] * $has_percent) / 100;
	...
	function AddAlienate($data = array()){
		...
		$sql = "select br.has_percent,br.valid_begin_time, bo.repayment_account,  bo.repayment_yesaccount from  {borrow_right}  as br,  {borrow}  as bo
		...
		if($data['price'] > $data['amount']) //高价转让
		{
		...
		if(time()-$right_result['valid_begin_time'] < $con_right_alienate_sday*3600*24)
		{
	...
	function get_biao_type_url_list($data = array()){
		...
		//liukun add for bug 52 begin
		fb($biao_type_list, FirePHP::TRACE);
		//liukun add for bug 52 end
		return $biao_type_list;
	...
	function getBiaoAuditInfo($data = array()){
		...
		if($biaotype_info['biao_credit_status'] == 1 && $credit_result['status']!=1){
			$msg = "发布借款标，必须完成征信认证。";
		}
...
// 		}
	...
	function getpactdate($data)
	{
		...
		{
			$string=file_get_contents('data/pact'.$_G['areaid'].'.html');
		}		
		
		$string=str_replace('璞胜金投网站',$_G['sitetitle'],$string);
		$string=str_replace('www.*****.com',$_SERVER['HTTP_HOST'],$string);
