...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		<tr >
            ...
            <td width="" class="main_td"><span title="借款人本金保障">本金保障</span></td>
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
            ...
            {if $item.biao_type == 'safety'}<span style="color:#FF0000">【保险标】</span>{/if}
            ...
            <td>{if $item.insurance ==1}
            		16%
            	{elseif $item.insurance ==2}
                	31%
                {elseif $item.insurance ==0}
                	无
                {else}
                	<span color='red'>数据异常</span>
            {/if}</td>
...
<div class="module_title"><strong>流标管理</strong></div>
	...
	<div class="module_border">
		...
		<div >
			<input type="text" name="days" value="{$_A.borrow_amount_result.account}" size="5" />天
		</div>
	</div>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		<tr >
            ...
            <td width="" class="main_td">垫付/本金保障</td>
		</tr>
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			...
			<td >{if $item.status==2}<font color="#FF0000">已代还</font>{else}{if $item.late_days>=0}<a href="{$_A.query_url}/late_repay{$_A.site_url}&id={$item.id}" onclick="return confirm('确定要垫付吗？')">垫付</a>{else}-{/if}{/if}
            
            {if $item.insurance !=0}
            		{if $item.insurance_status==0}
                    	<a href="{$_A.query_url}/insurance_do{$_A.site_url}&id={$item.id}" title="执行借款人本金保障" onclick="return confirm('确定执行借款人本金保障吗？')">执行</a>
                    {else}
                    	<span title="已执行借款人本金保障">已执行</span>
                    {/if}
                {/if}
            </td>
