...
<form action="{$_A.query_url}/order" method="post">
	<tr >
		...
		<td class="main_td">申请人姓名</td>
		<td class="main_td">申请人电话</td>
		<td class="main_td">申请时间</td>
		<td class="main_td">申请文档下载</td>
		<td class="main_td">操作</td>
	</tr>
	{ foreach  from=$_A.pioneer_list key=key item=item}
	<tr {if $key%2==1}class="tr2"{/if}>
		...
		<td><a href="{$item.doc_path}"  target="_blank" >点击下载</a></td>
		<td><a href="#" onClick="javascript:if(confirm('确定要删除吗?删除后将不可恢复')) location.href='{$_A.query_url}/del&id={$item.id}'">删除</a></td>
	</tr>
	{ /foreach}
	<tr>
