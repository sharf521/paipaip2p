...
<div class="module_add">
	...
	<div class="module_border">
		...
		<div class="c">
			<input type="text" name="url"  class="input_border" value="{ $_A.scrollpic_result.url}" size="30" />请填写图片对应的url地址，可为空
		</div>
	</div>
	
	<div class="module_border">
		<div class="l">图片说明：</div>
		<div class="c">
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
  <form action="{$_A.query_url}/order" method="post">
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center" >{if $item.pic!=""}<a href="./{$item.pic}" target="_blank"><img height="20" src="./{$item.pic}" border="0" /></a>{else}无图片{/if}</td>
		<td class="main_td1" align="center"  ><a href="{$_A.query_url}/edit&id={$item.id}">修改</a> / <a href="#" onClick="javascript:if(confirm('确定要删除吗?删除后将不可恢复')) location.href='{$_A.query_url}/del&id={$item.id}'">删除</a></div>
	</div>
	{ /foreach}
	<tr >
